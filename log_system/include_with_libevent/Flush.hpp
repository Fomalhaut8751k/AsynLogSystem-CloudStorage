...
namespace mylog
{
    ...
    {   // 将日志信息输出到控制台
    public:
        ConsoleFlush(std::string file_path= " ", size_t size = 1)
        {
    ...
    public:
        RollFileFlush(std::string file_path, size_t size)
        {
            ...
            for(;;)
            {
                ...
                {
                    std::cerr << "当前文件: " << new_file_path << \
                        ": (" << mylog::Util::File::FileSize(new_file_path) << "/" << size_ << ")" << std::endl;
                    new_file_path = "";
        ...
        void flush(const std::string& formatted_log)
        {
            ...
            */
            ...
            {
                std::cerr << "即将写入: " << current_file_path_ << std::endl;
                file_ << formatted_log << std::endl;
            }
            // 空间不足，需要创建新的.log
            else
