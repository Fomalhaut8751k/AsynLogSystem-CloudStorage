...
- 问题2
    ...
    ```
    经过多次测试发现，那些少数出现异常阻塞情况的，无一例外的当生产者的缓冲区越来越小以至于无法装下一条日志信息，但如果是正常的运行情况，如果生产者的缓存区有内容，它会交换和消费者的缓冲区，让消费者去处理日志信息，交换后的生产者缓冲区肯定是空的。因此我们可以推测：

    用户向生产者发送日志信息，此时消费者的缓冲区是空的，可以交换，但是，生产者并没有发生交换，任由自己的缓冲区越来越小，后面的用户提交日志信息时发现生产者的缓冲区空间不足，就等待在原地。

    有这么一种情况，原本生产者和消费者的缓冲区都是空的，此时有若干个用户来提交日志信息，第一个用户抢到锁后把日志写入，并声明有数据的标志，通知生产者起来干活，但是第一个用户释放锁后锁被第二个用户抢到了，第二个用户抢到后把日志写入，然后是同样的操作，如此，第n个用户抢到锁后想把日志写入时，发现生产者的缓冲区空间不足了，就等待。
