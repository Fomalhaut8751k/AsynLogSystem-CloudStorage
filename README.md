...
25. 2026.3.28
    ...
    | 1 KB | 81,474 | 46.0% | 10,409 ms | 848,061 |
...
26. 2026.3.29
    ...
    使用libevent实现一个简单的网页端压测，可以指定同步写入多少大小日志，结合ab实现高并发写入的测试

    由于异步日志系统启动在2核4G的ubuntu上，通过ab写入一个4G大小的日志，程序会终止并显示“已杀死”，由内存耗尽导致的程序崩溃。我们希望的是及时扼杀这条“恶意”的日志，保证日志系统不会崩溃(实测下来512M不会崩溃，但是单个缓冲区最大只能扩到64M)
